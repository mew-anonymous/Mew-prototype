...
class PortswitchTest(BfRuntimeTest):
	def setUp(self):
		...
		self.action_update_blocklist='SwitchIngress.update_blocklist'
		
		self.table_inform_others='SwitchIngress.inform_others'
	...
	def myconnect(self):
		...
		self.inform_others_table = self.bfrt_info.table_get(self.table_inform_others)
		...
		try:
			...
			self.thre_check_coremelt_table.entry_add(
				...
				[self.thre_check_coremelt_table.make_key(
					[gc.KeyTuple('flow_state', low=2048, high=65535)]
				)],
				[self.thre_check_coremelt_table.make_data(
					[],
					self.action_update_blocklist
				)]
			)
			self.inform_others_table.entry_add(
				self.target,
				[self.inform_others_table.make_key(
					[gc.KeyTuple('old_blockstate', 0)]
				)],
				[self.inform_others_table.make_data(
					[gc.DataTuple('dst_port', port_cpu)],
					self.action_upload_to_cpu_coremelt
				)]
			)
