...
control SwitchIngress(
	...
	action check_return1(){
		has_returned = req_ver_1.execute(record_key);
	}
	action check_return0(){
		has_returned = req_ver_0.execute(record_key);
	}
	table dup_filter{
		key = {
			cur_req_ver: exact;
	...
	/////////////flow_state//////////////////////////
		...
		action fstate1_read(){   // v: 0, g: 1, c: 0/1
			flow_state = flowrate1_read.execute(record_key)[15:0];//[31:16];
		}
		...
		action fstate2_read(){		// v: 0, g: 0, c: 0/1
			flow_state = flowrate2_read.execute(record_key)[15:0];//[31:16];
		}
