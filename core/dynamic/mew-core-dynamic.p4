...
Register<bit<32>, bit<32>>(32w1024) detection_state;
//flowsz/flow_rate
...
Register<bit<32>, bit<32>>(32w70000) pattern_state;
//link utilization
...
control SwitchIngress(
	...
	RegisterAction<bit<32>, bit<32>, bit<32>>(pattern_state) pattern_state_plus = {
        void apply(inout bit<32> value, out bit<32> read_value){
			read_value = value;
			value = value + pattern_state_toupdate;  
        }
    };
	RegisterAction<bit<32>, bit<32>, bit<32>>(pattern_state) pattern_state_read = {
        void apply(inout bit<32> value, out bit<32> read_value){
			read_value = value;
        }
    };
	RegisterAction<bit<32>, bit<32>, bit<32>>(pattern_state) pattern_state_clear = {
        void apply(inout bit<32> value, out bit<32> read_value){
			read_value = value;
			value = 0;
        }
	...
	//detection_state
	RegisterAction<bit<32>, bit<32>, bit<32>>(detection_state) detection_state_plus = {
        void apply(inout bit<32> value, out bit<32> read_value){
			read_value = value;
	...
	//flow_state_window1
		...
		action fstate1_congestion_read(){   // v: 0, g: 1, c: 0/1
			flow_congestion_state = flowrate1_congestion_read.execute(record_key);//[31:16];
		}
		...
		action fstate2_congestion_read(){		// v: 0, g: 0, c: 0/1
			flow_congestion_state = flowrate2_congestion_read.execute(record_key);//[31:16];
		}
	...
	action return_defense_info(bit<1> curv, bit<8> curt, bit<7> curid, bit<32> offset, bit<32> offset2){
		being_congested = 1;
		cur_req_ver = curv;
		cur_defense_type = curt;   //coremelt, crossfire, pulsing, ...
		cur_req_id = curid;
		cur_offset=offset; // x, y, z, ...
		cur_offset2=offset2; 
	}
	...
	action extract_coremelt(){
		extracted_res = pattern_res & 0xfff00000;
	}
	action extract_crossfire(){
		extracted_res = pattern_res & 0xfffff803;
	}
	action extract_pulsing(){
		extracted_res = pattern_res & 0xfff3ffff;
	}
	table extract_res{
		key = {
			cur_defense_type : exact;
		...
		if(is_blocked == 0 && hdr.ipv4.isValid()){ 
			...
			@stage(5){
				if(hdr.ipv4.version == 10)
					detection_state_toupdate = cur_offset2;  //passing_flow_num++
			}
			@stage(6){
				if(hdr.ipv4.version==4 || hdr.ipv4.version==5){
					if(link_state[31:10] == 0 && cur_defense_type != 0)  // link_load < 8Mbps or has been in defense mode
					{
			...
			@stage(6){
				if(flow_monitor_flag==1){
					if(cur_defense_type == 3){ //pulsing
						if(flow_congestion_state[31:10] == 0)
							;
						else{ 
			...
			@stage(10){
				detection_state_plus.execute(cur_port_index);
			}
			@stage(7){
				if (hdr.ipv4.version==9){
					flowkey_2tuple[16:0] = hash2.get({hdr.ipv4.src_addr, hdr.ipv4.dst_addr})[16:0];
					if (hdr.cd.req_id==my_swid[6:0]){
						if(flow_monitor_flag == 1){
							if(cur_defense_type == 1 && flow_state[31:10] == 0) //crossfire
								pattern_state_toupdate = cur_offset; //offset = 2^x
							else if(cur_defense_type == 3 && pulsing_flag == 1) //pulsing
								pattern_state_toupdate = cur_offset2; //offset = 2^y
							else if(cur_defense_type == 2) //coremelt
								pattern_state_toupdate[31:0] = fstate1_to_update; //offset = 0
							@stage(8){
								pattern_res = pattern_state_plus.execute(flowkey_2tuple);
							}
						}
							...
							if(cur_defense_type == 1 && hdr.cd.state1[0:0] == 1) //crossfire
								...
								pattern_state_toupdate[31:0] = fstate1_to_update;
							@stage(8){
								pattern_res = pattern_state_plus.execute(flowkey_2tuple);
							}
							hdr.cd.hit = 0;
