...
control SwitchIngress(
	...
	action fstate1_0_congestion_read(){
		flow_congestion_state = flowrate1_0_congestion_read.execute(record_key);
	}
	...
	action fstate2_0_congestion_read(){
		flow_congestion_state = flowrate2_0_congestion_read.execute(record_key);
	}
		...
		if(is_blocked == 0&& hdr.ipv4.isValid()){ 
			...
			@stage(5){
				if(hdr.ipv4.version==4 || hdr.ipv4.version==5){
					if(link_state[31:10] == 0 && cur_defense_type == 3)  // link_load < 8Mbps or has been in defense mode
					{
						meta.ing_mir_ses = 12;   
						ig_dprsr_md.mirror_type = 1;
					...
					else if (link_state[31:10] != 0 && cur_defense_type == 4){ //it's just congested
						meta.ing_mir_ses = 12;   
						ig_dprsr_md.mirror_type = 1;
						meta.edge_id=cur_port_index[7:0];
						meta.depth[1:1] = 1;
			...
			@stage(7){
				if (hdr.ipv4.version==9){
					if (hdr.cd.req_id==my_swid[6:0]){
						if((flow_monitor_flag==1 && pulsing_flag == 1) || (hdr.cd.hit==1 && hdr.cd.state1 == 1)){ //capture by itself
							flow_state4=flowrate4_plus.execute(record_key)[15:0];
							hdr.cd.hit=0;
						}
						else{ 
						...
						@stage(8){
							thre_check_pulsing.apply();
						}
					}
					else{//before and behind
						if(flow_monitor_flag==1 && pulsing_flag == 1){ //mask hdr.cd.hit
							hdr.cd.hit=1;
							hdr.cd.state1=1;
						}
						@stage(8){
